...
def reconstruct_from_partial(
    ...
    semantic_cands = space.semantic.search(frag_emb, top_k=max(1, _TOP_K // 2), relevance_weight=0.0)
    ...
    if candidates:
        ...
        neighbour_ids = space.graph.neighbors(top_node.id) if hasattr(space.graph, "neighbors") else []
